# Hopewick — Pilot pricing

**One-liner:** Hopewick keeps hope lit between sessions — an AI AOD recovery companion (Hope) for people in recovery. Staff issue invite codes; they are not the main users.

**Shareable HTML (prints well):** [`/pilot-pricing.html`](../pilot-pricing.html) → https://hopewick.com.au/pilot-pricing.html

**Email paste:** [`pilot-pricing-email.txt`](pilot-pricing-email.txt)

## Who it’s for

| | |
|---|---|
| Primary user | Clients / people in recovery (Hope between sessions) |
| Service role | Issue invite codes, brief people, decide whether to keep the program |
| Not | A clinician dashboard or a replacement for counselling |

## Free pilot (starting offer — evaluate fit)

| | |
|---|---|
| Duration | 4–6 weeks |
| Seats | 5–10 active invite codes (mostly clients; optional staff try-out codes) |
| Included | Company-held AI key, invite codes, fair usage caps, email support |
| Cost | Free for the pilot period |

## Paid plans after pilot (AUD / month, ex GST)

| Tier | Price | Client invites |
|------|------:|------:|
| Starter | $149 | up to 10 |
| Service | $349 | up to 40 |
| Multi-site | $749 | up to 100 across sites |

Bridge&Bite holds the AI key and caps; access is via invite codes. No in-app ads.

**GST:** AUD figures excluding GST. BRIDGE&BITE.CO PTY LTD is GST-registered (ABN 83 692 080 792); 10% GST is added on invoices.

**Contact:** admin@bridge-bite-co.com · BRIDGE&BITE.CO PTY LTD · ABN 83 692 080 792

**Disclaimer:** Not medical advice / not a crisis service. Emergency 000. National AOD Hotline 1800 250 015.
